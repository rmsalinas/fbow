...
public:
   ...
   */
  //EntryId add(const cv::Mat &features,
  //  fBow *bowvec = NULL, FeatureVector *fvec = NULL);
   ...
   *   < 0 means all
   */
  void query(const fBow &vec, QueryResults &ret, 
    int max_results = 1, int max_id = -1) const;
...
protected:
  ...
  void queryL1(const fBow &vec, QueryResults &ret, 
    int max_results, int max_id) const;
  
  /// Query with L2 scoring
  void queryL2(const fBow &vec, QueryResults &ret, 
    int max_results, int max_id) const;
  
  /// Query with Chi square scoring
  void queryChiSquare(const fBow &vec, QueryResults &ret, 
    int max_results, int max_id) const;
  
  /// Query with Bhattacharyya scoring
  void queryBhattacharyya(const fBow &vec, QueryResults &ret, 
    int max_results, int max_id) const;
  
  /// Query with KL divergence scoring  
  void queryKL(const fBow &vec, QueryResults &ret, 
    int max_results, int max_id) const;
  
  /// Query with dot product scoring
  void queryDotProduct(const fBow &vec, QueryResults &ret, 
    int max_results, int max_id) const;
