...
{
  fBow aux;
  fBow v;// = (bowvec ? *bowvec : aux);

    v = m_voc->transform(features); // with features
  
	/*
	for (std::pair<uint32_t, _float> e : v){
...
{
  fBow vec = m_voc->transform(features);
...
void Database::queryL1(const fBow &vec,
  QueryResults &ret, int max_results, int max_id) const
{
  fBow::const_iterator vit;
...
void Database::queryL2(const fBow &vec,
  QueryResults &ret, int max_results, int max_id) const
{
  fBow::const_iterator vit;
...
void Database::queryChiSquare(const fBow &vec,
  QueryResults &ret, int max_results, int max_id) const
{
  fBow::const_iterator vit;
...
void Database::queryKL(const fBow &vec,
  QueryResults &ret, int max_results, int max_id) const
{
  fBow::const_iterator vit;
...
void Database::queryBhattacharyya(
  const fBow &vec, QueryResults &ret, int max_results, int max_id) const
{
  fBow::const_iterator vit;
...
void Database::save(const std::string &filename) const
{
  ...
  m_voc->saveToFile(std::string("voc_") + filename);
...
void Database::load(const std::string &filename)
{
  ...
  Vocabulary voc;
  voc.readFromFile(std::string("voc_") + filename);
  setVocabulary(voc);
